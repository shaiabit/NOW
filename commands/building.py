...
commit 4b9db9570c668ea631702c1188d9cc8796633d0d
"""
...
class CmdCreate(ObjManipCommand):
    ...
    new_obj_lockstring = "control:id({id}) or perm(wizard);delete:id({id}) or perm(wizard)"
...
class CmdDig(ObjManipCommand):
    ...
    # Will be formatted with the {id} of the creating object.
    new_room_lockstring = "control:id({id}) or perm(wizard); " \
                          "delete:id({id}) or perm(wizard); " \
                          "edit:id({id}) or perm(wizard)"
    ...
    def func(self):
        ...
        try:
            ...
            string = ("|RCritical Python syntax error in argument. "
                      "Only primitive Python structures are allowed. "
                      "\nYou also need to use correct Python syntax. "
                      "Remember especially to put quotes around all "
        ...
        if isinstance(prototype, basestring):
            ...
            # The prototype on the command line must make sure to not allow
            # the 'exec' key unless permission is immortal or higher.
            if "exec" in prototype and not self.caller.check_permstring("immortal"):

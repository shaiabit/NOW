...
# of a object reference, such as /tall
    ...
    for marker_match in reversed(list(_RE_OBJ_REF_START.finditer(string))):
        ...
        if not nmatches:
            ...
            if inum is not None:
                ...
                nmatches = 1
            else:  # no identifier given - a real multimatch.
                obj = bestmatches
        if search_mode:  # Single-object search mode. Don't continue loop.
            break
        elif nmatches == 0:
...
class RecogHandler(object):
    ...
    def _cache(self):
        ...
        self.obj2regex = dict((obj, re.compile(regex, _RE_FLAGS)) for obj, regex in obj2regex.items() if obj)
    ...
    def add(self, obj, recog, max_length=60):
        ...
        Raises:
            SdescError: When recog could not be set or sdesc longer than
            `max_length`.
        """
    ...
    def get(self, obj):
        ...
        """
        if not self.obj.location.tags.get('rp', category='flags'):
            return obj.key
        ...
        else:  # recog_mask log not passed, disable recog
...
class CmdSdesc(RPCommand):  # set/look at own sdesc
    ...
    Assigns a short description to yourself as others would identify you.
    """
    key = 'sdesc'
    locks = 'cmd:all()'
    help_category = 'Rollplay'
...
class CmdRoomPose(RPCommand):  # set current pose and default pose
    ...
    Examples:
        rp leans against the tree
        rp is talking to the barkeep.
        rp box = is sitting on the floor.
    ...
    def func(self):
        ...
        if self.reset:
            ...
            mapping = dict((ref, obj.sdesc.get() if hasattr(obj, '_sdesc') else obj.key)
...
class CmdRecog(RPCommand):  # assign personal alias to object in room
    ...
    This will assign a personal alias for a person, or forget the alias.
    """
    key = 'recog'
        ...
        if nmatches == 0:
            ...
            if self.cmdstring == 'forget':  # remove existing recog
                ...
                try:
                    ...
                    caller.msg(err)
                    return
                caller.msg('%s will now remember |w%s|n as |w%s|n.' % (caller.get_display_name(caller), sdesc, alias))
...
class CmdMask(RPCommand):
    ...
    def func(self):
        caller = self.caller
        if self.cmdstring == 'mask':  # wear a mask
            if not self.args:
                caller.msg('Usage: (un)mask <sdesc>')  # Return to your last disguise.
                return
            if caller.db.unmasked_sdesc:
                caller.msg('You are already masked.')
                return
            sdesc = _RE_CHAREND.sub('', self.args)
            ...
            if not old_sdesc:
                caller.msg('You are not disguised.')
                return
...
class RPObject(DefaultObject):
    ...
    def search(self, searchdata,
        ...
        if use_nicks:  # do nick-replacement on search
        ...
        if(global_search or (is_string and searchdata.startswith('#') and
            ...
            else:  # Local search.  #  Candidates are taken from:
                location = self.location  # self.location,
                candidates = self.contents  # self.contents,
                if location:  # and self.location.contents.
                    candidates = candidates + [location] + location.contents
                else:  # If not location, then only self (Search in Nothingness)
        ...
        if candidates:
            ...
            print('Results of search: %s' % results)
